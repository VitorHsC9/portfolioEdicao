import { memo, useMemo, useCallback, useRef } from 'react';
import { motion, useInView } from 'framer-motion';
import { trackPortfolioClick } from '../../utils/analytics';
import './Portfolio.css';

/**
 * Portfolio section displaying works in a grid
 * Memoized to prevent unnecessary re-renders
 */
const Portfolio = memo(() => {
    const ref = useRef(null);
    const isInView = useInView(ref, { once: true, margin: '-50px' });

    // Memoized static works data
    const works = useMemo(() => [
        {
            id: 1,
            title: 'El Árbol de la Vida',
            category: 'Edição de Vídeo',
            description: 'Mística da existência humana',
            image: '/assets/portfolio/1.jpg',
            videoUrl: 'https://www.instagram.com/reel/DRdKFVXgEQE/',
        },
        {
            id: 2,
            title: 'Saint Augustine',
            category: 'Edição de Vídeo',
            description: 'Interioridade, fé e razão',
            image: '/assets/portfolio/2.jpg',
            videoUrl: 'https://www.instagram.com/reel/DPRU_yBAIkm/',
        },
        {
            id: 3,
            title: 'Cristo Pantocrator',
            category: 'Edição de Vídeo',
            description: 'Autoridade divina absoluta',
            image: '/assets/portfolio/3.jpg',
            videoUrl: 'https://www.instagram.com/reel/DSNm9FmAJE7/',
        },
        {
            id: 4,
            title: 'Emoções na Arte',
            category: 'Edição de Vídeo',
            description: 'Capturando emoções na arte',
            image: '/assets/portfolio/4.jpg',
            videoUrl: 'https://www.instagram.com/reel/DQ_wBjdAK0d/',
        },
        {
            id: 5,
            title: 'A Incredulidade de São Tomé',
            category: 'Edição de Vídeo',
            description: 'Fé tocando a dúvida',
            image: '/assets/portfolio/5.jpg',
            videoUrl: 'https://www.instagram.com/reel/DRk3LuwiP_k/',
        },
        {
            id: 6,
            title: 'Arte Barroca',
            category: 'Edição de Vídeo',
            description: 'Êxtase, drama e movimento',
            image: '/assets/portfolio/6.jpg',
            videoUrl: 'https://www.instagram.com/reel/DPWNtHBgILy/',
        },
    ], []);

    // Memoized animation variants
    const containerVariants = useMemo(() => ({
        hidden: { opacity: 0 },
        visible: {
            opacity: 1,
            transition: {
                staggerChildren: 0.15,
                delayChildren: 0.2,
            },
        },
    }), []);

    const cardVariants = useMemo(() => ({
        hidden: { opacity: 0, y: 50 },
        visible: {
            opacity: 1,
            y: 0,
            transition: { duration: 0.8, ease: 'easeOut' },
        },
    }), []);

    const headerVariants = useMemo(() => ({
        hidden: { opacity: 0, y: 30 },
        visible: {
            opacity: 1,
            y: 0,
            transition: { duration: 0.8 },
        },
    }), []);

    // Memoized click handler with analytics tracking
    const handleWorkClick = useCallback((videoUrl, title) => {
        trackPortfolioClick(title);
        window.open(videoUrl, '_blank', 'noopener,noreferrer');
    }, []);

    return (
        <section className="portfolio section" id="obras" ref={ref}>
            <div className="container">
                <motion.div
                    className="portfolio-header"
                    variants={headerVariants}
                    initial="hidden"
                    animate={isInView ? 'visible' : 'hidden'}
                >
                    <span className="portfolio-eyebrow">Portfólio</span>
                    <h2 className="portfolio-title">Obras Selecionadas</h2>
                    <p className="portfolio-subtitle">
                        Clique para assistir cada obra
                    </p>
                </motion.div>

                <motion.div
                    className="portfolio-grid"
                    variants={containerVariants}
                    initial="hidden"
                    animate={isInView ? 'visible' : 'hidden'}
                >
                    {works.map((work) => (
                        <motion.article
                            key={work.id}
                            className="portfolio-card"
                            variants={cardVariants}
                            whileHover={{ y: -5 }}
                            onClick={() => handleWorkClick(work.videoUrl, work.title)}
                        >
                            <img
                                src={work.image}
                                alt={work.title}
                                className="portfolio-card-image"
                                loading="lazy"
                            />
                            <div className="portfolio-card-overlay">
                                <div className="portfolio-card-content">
                                    <span className="portfolio-card-category">{work.category}</span>
                                    <h3 className="portfolio-card-title">{work.title}</h3>
                                    <p className="portfolio-card-description">{work.description}</p>
                                    <span className="portfolio-card-play">▶ Assistir</span>
                                </div>
                            </div>
                            <span className="portfolio-card-ornament">✦</span>
                        </motion.article>
                    ))}
                </motion.div>
            </div>
        </section>
    );
});

Portfolio.displayName = 'Portfolio';

export default Portfolio;

